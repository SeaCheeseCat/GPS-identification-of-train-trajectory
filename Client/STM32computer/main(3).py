...
class GNSSWindow(QWidget):
    def __init__(self):
        ...
        self.seltrajectories = [
            ((300, 300), (300, 250), (300, 200), (300, 150), (300, 100), (300, 50)),  # 线段1
            ((300, 300), (300, 250), (300, 200), (350, 200), (400, 200)),  # 线段2
            ((300, 300), (300, 250), (300, 200), (350, 200), (400, 200), (450, 150), (450, 100), (450, 50)),  # 线段2
        ]
    ...
    def paintEvent(self, event):
        ...
        #self.current_position =( -0.00008, -0.00008)
        # 绘制当前位置
        if self.current_position:
            ...
            self.draw_turnRight_indicator(painter)
    ...
    def draw_turnRight_indicator(self, painter):
        if self.current_position is not None:
            pen = QPen(Qt.blue, 2)
            painter.setPen(pen)

            # 箭头长度和宽度
            arrow_length = 0.000015
            arrow_width = 0.000005

            angle = 0  # 默认角度

            angle = -math.pi*2   # 右转-90度

            # 计算箭头的主线结束点
            end_x = self.current_position[0] + arrow_length * math.cos(angle)
            end_y = self.current_position[1] + arrow_length * math.sin(angle)

            # 绘制箭头的主线
            painter.drawLine(
                int(self.current_position[0] * self.scale + self.width() // 2),
                int(self.current_position[1] * self.scale + self.height() // 2),
                int(end_x * self.scale + self.width() // 2),
                int(end_y * self.scale + self.height() // 2)
            )

            du = 0.95
            # 计算箭头两侧的坐标
            left_x = end_x + arrow_width * math.cos(angle - math.pi / 6 - math.pi / du)  # 向外翻折
            left_y = end_y + arrow_width * math.sin(angle - math.pi / 6 - math.pi / du)  # 向外翻折
            right_x = end_x + arrow_width * math.cos(angle + math.pi / 6 - math.pi / du)  # 向外翻折
            right_y = end_y + arrow_width * math.sin(angle + math.pi / 6 - math.pi / du)  # 向外翻折

            # 绘制箭头两侧
            painter.drawLine(int(end_x * self.scale + self.width() // 2), int(end_y * self.scale + self.height() // 2),
                             int(left_x * self.scale + self.width() // 2),
                             int(left_y * self.scale + self.height() // 2))
            painter.drawLine(int(end_x * self.scale + self.width() // 2), int(end_y * self.scale + self.height() // 2),
                             int(right_x * self.scale + self.width() // 2),
                             int(right_y * self.scale + self.height() // 2))
    ...
    def draw_trajectories(self, painter):
        ...
        if self.selected_trajectory is not None:
            secttrajectory = self.seltrajectories[self.selected_trajectory]
            pen = QPen(QColor(0, 0, 255), 2)  # 选中状态：蓝色
            painter.setPen(pen)
            for j in range(len(secttrajectory) - 1):
                start_point = secttrajectory[j]
                end_point = secttrajectory[j + 1]
                painter.drawLine(int(start_point[0]), int(start_point[1]), int(end_point[0]), int(end_point[1]))
        ...
        for i, trajectory in enumerate(self.trajectories):
            for j in range(len(trajectory) - 1):
                ...
                if self.is_point_near_line(click_pos, start_point, end_point):
                    self.selected_trajectory = i  # 记录选中的轨迹
                    self.update()  # 重新绘制
                    return
